...
main_exe = (base_folder / 'Wplace-GUI.exe').resolve()
image_process_exe = (base_folder / 'image_process.exe').resolve()
config_GUI_exe = (base_folder / 'config_GUI.exe').resolve()

template = (base_folder / 'template.png').resolve()

timeline_folder = (base_folder / 'timeline').resolve()
timeline = find_last_one(timeline_folder, r'^(\d{8})_(\d{6}).png$')

timeline_cropped_folder = (base_folder / 'timeline_cropped_png').resolve()
timeline_cropped = find_last_one(timeline_cropped_folder, r'^(\d{8})_(\d{6}).png$')

timeline_color_folder = (base_folder / 'timeline_color').resolve()
...
if __name__ == "__main__":
    ...
    print(f"Latest color file in color folder: {timeline_color_list}")
